    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-surface border border-surface-border flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-wider text-slate-400 font-mono font-semibold">Total Audited</span>
            <div className="text-2xl font-bold font-mono text-white mt-1">{products.length}</div>
            <span className="text-[11px] text-slate-400">Indexed master specs</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-emerald-500/30 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-wider text-slate-400 font-mono font-semibold">Passed Rules</span>
            <div className="text-2xl font-bold font-mono text-emerald-400 mt-1">{validatedCount}</div>
            <span className="text-[11px] text-emerald-400 font-mono">100% Conforming</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-rose-500/30 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-wider text-slate-400 font-mono font-semibold">Active Conflicts</span>
            <div className="text-2xl font-bold font-mono text-rose-400 mt-1">{conflictCount}</div>
            <span className="text-[11px] text-rose-300 font-mono">Needs Engineer Review</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-surface border border-amber-500/30 flex items-center justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-wider text-slate-400 font-mono font-semibold">Warnings / Gaps</span>
            <div className="text-2xl font-bold font-mono text-amber-400 mt-1">{anomalyCount}</div>
            <span className="text-[11px] text-amber-300 font-mono">Non-blocking anomalies</span>
          </div>
      ...
      {/* 2. Filter Tabs */}
      <div className="flex items-center gap-2 p-1 rounded-lg bg-surface border border-surface-border w-fit">
        <button
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md text-xs font-semibold transition-all flex items-center gap-1.5 ${
            activeTab === 'CONFLICT' ? 'bg-rose-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md text-xs font-semibold transition-all flex items-center gap-1.5 ${
            activeTab === 'ANOMALY' ? 'bg-amber-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
      ...
      {activeTab === 'RULES' ? (
        <div className="bg-surface rounded-2xl border border-surface-border p-6 shadow-xl space-y-4">
          <div className="pb-3 border-b border-surface-border">
            <h3 className="text-base font-bold text-white tracking-tight">Active Deterministic Rule Catalog</h3>
            <p className="text-xs text-slate-400 font-mono">Enforced across all incoming technical documents and catalog records</p>
          </div>
          ...
          <div className="space-y-3">
            {activeRulesCatalog.map((r) => (
              <div
                key={r.id}
                className="p-4 rounded-xl bg-surface-elevated border border-surface-border flex items-start justify-between gap-4"
              >
                ...
                <span
                  className={`font-mono text-[10px] font-bold px-2.5 py-1 rounded border shrink-0 ${
                    r.passed
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="bg-surface rounded-2xl border border-surface-border p-12 text-center space-y-3">
              ...
              <h3 className="text-base font-bold text-white">No Validation Issues Found</h3>
