...
export const ValidationBadge: React.FC<ValidationBadgeProps> = ({ status, size = 'md' }) => {
  const config = {
    ...
    CONFLICT: {
      ...
      icon: AlertTriangle,
      style: 'bg-rose-500/10 text-rose-400 border-rose-500/30 font-semibold animate-pulse'
    },
    ANOMALY: {
      label: 'Issue / Anomaly',
      icon: AlertCircle,
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-md border font-medium ${sizeClasses} ${config.style}`}
    >
